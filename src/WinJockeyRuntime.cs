...
namespace Mastersign.WinJockey
{
    partial class WinJockeyRuntime : IDisposable
    {
        ...
        {
            Actions = new Actions() { Runtime = this };
        ...
        public void ShowDebugMessage(string msg) => Dispatcher.BeginInvoke((Action<string>)AddDebugMessageToMessageList, msg);
        ...
        {
            configObserver = new PropertyObserver<WinJockeyRuntime, WinJockeyConfiguration>(
                this, nameof(Config), BindConfig, UnbindConfig);
            mqttConnectionObserver = new PropertyObserver<WinJockeyRuntime, MqttConnection>(
                this, nameof(MqttConnection), BindMqttConnection, UnbindMqttConnection);
        }
        ...
        {
            setupObserver = new PropertyObserver<WinJockeyConfiguration, WinJockeySetup>(
                configuration, nameof(WinJockeyConfiguration.Setup), BindSetup, UnbindSetup);
        }
        ...
        {
            mqttSetupObserver = new PropertyObserver<WinJockeySetup, MqttSetup>(
                setup, nameof(WinJockeySetup.MqttServer), BindMqttSetup, UnbindMqttSetup);
        }
        ...
        private void MqttMessageHandler(object sender, MqttEventArgs e)
        {
            foreach (var command in MatchMqttMessage(e))
            {
                ...
                Dispatcher.BeginInvoke((Action<CommandConfiguration>)Actions.Trigger, command);
            }
        }

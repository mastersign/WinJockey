...
namespace Mastersign.WinJockey
{
    public sealed class MqttConnection : IDisposable
    {
        ...
        private static TimeSpan connectInterval = TimeSpan.FromSeconds(5);
        private DateTime lastConnectionAttempt = DateTime.MinValue;

        private readonly BlockingCollection<MqttEventArgs> mqttEvents = new BlockingCollection<MqttEventArgs>(new ConcurrentQueue<MqttEventArgs>());
        ...
        private async Task ConnectionWorker()
        {
            ...
            while (!disposeToken.IsCancellationRequested)
            {
                ...
                var threshold = lastConnectionAttempt + connectInterval;
                if (now < threshold)
                {
                    await Task.Delay(threshold - now);
                }
